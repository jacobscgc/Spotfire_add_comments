...
if len(ids) == 0:
	...
	# Loop through the selected rows and identify which values need to be replaced with the current comment:
	...
	for currentOverrideValue, row_id_col_values, today_override_value in update_list_of_lists:
		...
		# If the comment did not change since the last refresh of the User, add the comment and comment modification date:
			...
			transformations.Add(ReplaceSpecificValueTransformation(column, currentOverrideValue, "{} ({})".format(Comment, User), row_id_cols, row_id_col_values, False))  # Replace comment
	...
	Document.Properties["Message"] = "{} UTC | Duration: {} seconds | Comment '{}' was added.".format(today, round((time.time() - time0), 2), Comment)
