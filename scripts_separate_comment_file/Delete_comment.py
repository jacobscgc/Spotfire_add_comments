...
def delete_comments():
	"""
	This function loops through the ids where the Comment should be deleted. For each id, check whether there was already a comment. If so, check whether the comment
	in the comment table is the same as in the file or whether it was changed by a different user in the meanwhile. If it didn't change, delete it. Otherwise ignore it.
	...
	not_deleted = ''
	for id_entry in ids:
		if id_entry in current_val_dict:  # there was already a comment for this batch
			# Ignore the match date when comparing because it is not used.
			if current_comment == '':
				current_comment = current_val_dict[id_entry][0]
			else:
				current_comment = current_comment + ' | ' + current_val_dict[id_entry][0]  # add the comment that is going to be deleted to string
			print current_val_dict[id_entry][0:2], comment_dict[id_entry][0:2]
			if current_val_dict[id_entry][0:2] == comment_dict[id_entry][0:2]:  # spotfire table and macaw file have same entry, delete:
				del comment_dict[id_entry]
			else:
				failed += 1
				if not_deleted == '':
					not_deleted = current_val_dict[id_entry][0]
				else:
					not_deleted = not_deleted + ' | ' + current_val_dict[id_entry][0]
				print "Comment in comment table differs from the one in the file for batch {}, not deleted.".format(id_entry)
		else:  # Comment didn't exist yet, cannot delete (shouldn't be possible to get to this code):
			failed += 1
			print "Comment {} not found, cannot delete.".format(id_entry) 
	...
	if failed == 0:
		Document.Properties["Message"] = "{} UTC | Duration: {} seconds | Comment(s) '{}' was/were deleted for {} selected rows.".format(today, round((time.time() - time0), 2), current_comment, batch_number)
	else:
		Document.Properties["Message"] = "{} UTC | Duration: {} seconds | Unable to delete the comment(s) '{}' for {} out of {} selected rows due to updated source file or non-existing comment.".format(today, round((time.time() - time0), 2), not_deleted, failed, batch_number)
...
# Try to delete the comments, if the output file is busy, wait 10 seconds and try again. If it cannot process it within 3 attempts, abort and write a message to the User that the file is busy.
try:
	delete_comments()
except:
	time.sleep(10)
	try:
		...
		try:
			...
			today = datetime.utcnow().strftime("%d/%m/%Y %H:%M:%S")
			Document.Properties["Message"] = "{} UTC | Duration: {} seconds | Unable to delete comments, output file in use.".format(today, round((time.time() - time0), 2))
