...
def add_replace_comments():
	"""
	This function loops through the ids where the Comment should be added or replaced. For each id, check whether there was already a comment. If so, check whether the comment
	...
	for id_entry in ids:
		if id_entry in current_val_dict:  # there was already a comment for this batch
			...
			if current_val_dict[id_entry][0:2] == comment_dict[id_entry][0:2]:  # spotfire table and file have same entry, replace:
				mod_date_new = datetime.utcnow().strftime("%d/%m/%Y %H:%M:%S")  # update timestamp for modification date
				match_date = datetime.utcnow().strftime("%d/%m/%Y")  # update match date
				comment_dict[id_entry] = [Comment, User, mod_date_new, match_date]  # update entry in dict
			else:
				failed += 1
				print "Comment in comment table differs from the one in the file for batch {}, not updated.".format(id_entry)
	...
	if failed == 0:
		Document.Properties["Message"] = "{} UTC | Duration: {} seconds | Comment '{}' was added/replaced to {} selected rows.".format(today, round((time.time() - time0), 2), Comment, batch_number)
	else:
		Document.Properties["Message"] = "{} UTC | Duration: {} seconds | Unable to add/replace comment '{}' for {} out of {} selected rows due to updated source file.".format(today, round((time.time() - time0), 2), Comment, failed, batch_number)
